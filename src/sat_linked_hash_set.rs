...
impl SatLinkedHashSet {
    pub fn new(variable_connections: &Vec<Vec<usize>>) -> Self {
        ...
        for i in 0..num_vars {
            let mut var_list: Vec<SatNode> = Vec::<SatNode>::with_capacity(5000);
            let mut var_set: Vec<usize> = Vec::<usize>::with_capacity(5000);
            let mut var_open_spots: Vec<usize> = Vec::<usize>::with_capacity(5000);
            let mut var_open_spots_len: usize = 0;
            let head: usize = 0;
            // don't forget there is a 5000 in len()
            for j in 0..5000 {
                var_open_spots.push(5000 - j - 1);
    ...
    // couldn't figure out the borrow checker, so now 5000 is hard coded in solver.rs
    // pub fn len(self, var: usize) -> usize {
    //     5000 - self.open_spots_len[var]
    // }
    ...
    pub fn update_open_spots_remove(&mut self, var: usize, spot_will_be_open: usize) {
        // if self.open_spots_len[var] == 5000 {
        //     println!("{}", var);
        // }
