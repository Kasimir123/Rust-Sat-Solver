...
impl SatLinkedHashSet {
    pub fn new(variable_connections: &Vec<Vec<usize>>) -> Self {
        ...
        for i in 0..num_vars {
            let mut var_list: Vec<SatNode> = Vec::<SatNode>::with_capacity(500000);
            let mut var_set: Vec<usize> = Vec::<usize>::with_capacity(500000);
            let mut var_open_spots: Vec<usize> = Vec::<usize>::with_capacity(500000);
            let mut var_open_spots_len: usize = 0;
            let head: usize = 0;
            // don't forget there is a 500000 in len()
            for j in 0..500000 {
                var_open_spots.push(500000 - j - 1);
    ...
    // couldn't figure out the borrow checker, so now 500000 is hard coded in solver.rs
    // pub fn len(self, var: usize) -> usize {
    //     500000 - self.open_spots_len[var]
    // }
    ...
    pub fn update_open_spots_remove(&mut self, var: usize, spot_will_be_open: usize) {
        // if self.open_spots_len[var] == 500000 {
        //     println!("{}", var);
        // }
